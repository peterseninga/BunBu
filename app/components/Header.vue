...
const handleThemeSelection = (selection: { item: string, category: string }) => {
  console.log('Thema gewählt:', selection)
  
  // Navigation zu Filterseite mit Thema
  const slug = selection.item.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '')
  navigateTo({
    path: '/filter',
    query: { 
      theme: slug,
      category: selection.category.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '')
    }
  })
}
...
const customFormats = computed(() => {
  ...
  return filternav.value?.data.format || [
    { name: 'Buch', count: 6 },
...
<template>
    <Bounded as="header" class="!py-4 md:!py-6 lg:!py-8 bg-[#004b5a] text-white">
        <div class="flex flex-col items-center justify-between sm:flex-row gap-4">
            ...
            <div class="flex-1 flex justify-center order-3 sm:order-2">
                <MegaDropdown 
            ...
            <div class="w-xl sm:w-auto sm:ml-auto order-2 sm:order-3">
                <SearchBar />
            </div>
        </div>
    </Bounded>
